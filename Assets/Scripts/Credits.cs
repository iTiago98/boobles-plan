...
public class Credits : MonoBehaviour
{
    ...
    public class Entry
    {
        ...
        [field: SerializeField] public List<string> Participants { get; private set; }
    }
    
    [SerializeField] private List<Entry> _entries;
    [SerializeField] private List<Entry> _jokeEntries;
    [SerializeField] private List<Entry> _thanksEntries;
    
    [Header("Templates")]
    [SerializeField] private GameObject _title;
    [SerializeField] private GameObject _spaceTitleParticipant;
    [SerializeField] private GameObject _participant;
    [SerializeField] private GameObject _spaceParticipants;
    [SerializeField] private GameObject _spaceEntries;
    [SerializeField] private GameObject _spaceSections;
    [Header("Values")]
    ...
    private void Awake()
    {
        ...
        PrintEntry(_entries);
        Instantiate(_spaceSections, transform);
        PrintEntry(_jokeEntries);
        Instantiate(_spaceSections, transform);
        PrintEntry(_thanksEntries);
    }
    ...
    private void PrintEntry(List<Entry> entries)
    {
        ...
        {
            Instantiate(_title, transform).GetComponent<TMP_Text>().text = entry.Title;
            Instantiate(_spaceTitleParticipant, transform);
            foreach (var participant in entry.Participants)
            {
                Instantiate(_participant, transform).GetComponent<TMP_Text>().text = participant;
                Instantiate(_spaceParticipants, transform);
            }
            Destroy(transform.GetChild(transform.childCount-1).gameObject);
            Instantiate(_spaceEntries, transform);
        }
    }
